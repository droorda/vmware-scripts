...
Function xMove-VM {
    ...
    # ERRORS in PS 7
    if (($TrustAllCerts) -and ($PSVersionTable.PSEdition -eq 'Desktop')) {
        $TypeDefinition = @"
        using System.Net;
...
"@
    ...
    if ($PSVersionTable.PSEdition -eq 'Desktop') {
        ...
        $destVCThumbprint = (Get-WebCertificate -FQDN $destVC).Thumbprint
    }
    ...
    Try {
        ...
        $task1 = Get-Task -Id $task.ToString() -Server $sourcevc -ErrorAction Stop
        $task1 | Wait-Task -ErrorAction Stop
    } catch {
        Write-Warning $_.Exception.Message
        Write-Verbose "spec`n$($spec | Format-List | Out-String)" -Verbose
        Write-Verbose "task`n$($task.ToString())`n$($task | Format-List | Out-String)" -Verbose
    }
    if ($task1.ExtensionData.Info.State -eq "success") {
        Write-Host "VM $vmname has been successfully migrated to $destVC"
        Get-VM -Server $destVC -Name $vm.Name
    } else {
        Write-Host "VM $vmname migration $($task1.ExtensionData.Info.State): $($task1.ExtensionData.Info.Error.LocalizedMessage)"
        $vm
    }
}
